...
class Vehicle {
  ...
  constructor(x, y,isFollower=false) {
    ...
    this.maxForce = 0.9; 
    this.color = color(random(255), random(255), random(255)); // Couleur aléatoire
  ...
  evadeLeaderZone(leaderPos, leaderHeading, zoneWidth) {
    let evadePoint = this.calculateEvadePoint(leaderPos, leaderHeading, zoneWidth);
    this.drawEvadeZone(evadePoint, zoneWidth);
  
    // Calculer la distance entre le véhicule et le point d'évasion
    let distanceToEvadePoint = this.pos.dist(evadePoint);
  
    // Si la distance est inférieure à la demi-largeur de la zone
    let halfZoneWidth = zoneWidth / 2;
    if (distanceToEvadePoint < halfZoneWidth + this.r) {
      // Éviter le point d'évasion (comportement d'évasion inverse de pursuit)
      let evadeForce = this.evade(evadePoint);
      this.applyForce(evadeForce);
    } else {
      // En dehors de la zone, mettre le poids du comportement d'évasion à zéro
      this.evadeWeight = 0;
    }
  }
  ...
  // seek et avoid
 // Ajoutez cette constante pour définir la largeur de la zone d'évasion devant le leader

// Modifiez la méthode applyBehaviors dans la classe Vehicle
// Dans la classe Vehicle, modifiez la fonction applyBehaviors comme suit :
applyBehaviors(target, obstacles, vehicles, separationDistance) {
  let fleeForce = this.flee(target);

  // Ajoutez la fuite des obstacles
  let avoidObstaclesForce = this.avoidObstacles(obstacles);

  // Ajoutez la séparation des autres véhicules
  let separateForce = this.separate(vehicles, separationDistance);

  // Ajoutez ces forces à la force totale
  this.applyForce(fleeForce);
  this.applyForce(avoidObstaclesForce);
  this.applyForce(separateForce);
}

// Ajoutez également une fonction flee à la classe Vehicle :
flee(target) {
...
// Et ajoutez une fonction avoidObstacles à la classe Vehicle :
avoidObstacles(obstacles) {
  let desiredSeparation = this.r * 2;
  let sum = createVector(0, 0);
  let count = 0;
  for (let i = 0; i < obstacles.length; i++) {
    let d = p5.Vector.dist(this.pos, obstacles[i].pos);
    if (d < desiredSeparation) {
      let diff = p5.Vector.sub(this.pos, obstacles[i].pos);
      diff.normalize();
      diff.div(d);
      sum.add(diff);
      count++;
...
drawZoneAhead() {
  ...
  // Dessiner la zone
  fill(255, 225, 0, 50); // Couleur semi-transparente pour la zone
  noStroke();
  circle(pointAhead.x, pointAhead.y, 80); 
}
...
calculateEvadePoint(leaderPos, leaderHeading, zoneWidth) {
  // Calculer le point devant le leader avec une largeur de zone
  let evadePoint = p5.Vector.add(leaderPos, p5.Vector.mult(leaderHeading, zoneWidth));
  return evadePoint;
}
drawEvadeZone(evadePoint, zoneWidth) {
  // Dessiner une ligne bleue devant le véhicule pour représenter la zone d'évasion
  stroke(0, 0, 255); // Couleur bleue
  strokeWeight(2);
  line(this.pos.x, this.pos.y, evadePoint.x, evadePoint.y);
}
...
evadeLeaderZone(leaderPos, leaderHeading, zoneWidth) {
  ...
  drawVehicle() {
    // Set the fill color based on whether the vehicle is a leader or follower
    if (this.isLeader) {
      fill(0, 0, 255); // Leader is blue
    } else {
      fill(255, 165, 0); // Followers are orange
    }
    ...
    push();
    // Translate to the position of the vehicle
    translate(this.pos.x, this.pos.y);
    // Draw a circle instead of a triangle
    ellipse(0, 0, this.r_pourDessin * 2);
  
    // Draw velocity vector
    this.drawVector(this.pos, this.vel, color(255, 0, 0));
  
    // Cercle pour évitement entre véhicules et obstacles (debug)
    if (Vehicle.debug) {
      stroke(255);
      noFill();
      circle(0, 0, this.r);
